...
function errorIfMissingConfiguration(config, requiredOptions, errorPrefix) {
	const givenOptions = Object.keys(config);
	const missingOptions = requiredOptions.filter(
		requiredOption => !givenOptions.includes(requiredOption)
	);
	if (missingOptions.length > 0) {
		throw new Error(`${errorPrefix}. Missing configuration: "${missingOptions.join('", "')}".`);
	}
}

module.exports.js = function(gulp, config) {
	config = config || {};
	errorIfMissingConfiguration(
		config,
		['cwd', 'buildFolder', 'js'],
		'Unable to build JavaScript',
	);
	const src = config.js || null;
...
module.exports.sass = function(gulp, config) {
	...
	errorIfMissingConfiguration(
		config,
		['cwd', 'buildFolder', 'sass'],
		'Unable to build Sass',
	);
	const src = config.sass || null;
	const cwd = config.cwd;
