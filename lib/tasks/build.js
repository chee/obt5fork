...
module.exports.js = function(gulp, config) {
	...
	log.secondary('Webpacking ' + config.js);
...
module.exports.sass = function(gulp, config) {
	...
	log.secondary('Compiling ' + config.sass);

	config.env = config.env || 'development';
	const useSourceMaps = config.sourcemaps || (config.env === 'development');

	const sassConfig = {
		includePaths: [path.join(config.cwd, 'bower_components')].concat(config.sassIncludePaths || []),
		outputStyle: 'nested'
	};
